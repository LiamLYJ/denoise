...
def bilerp(image, grid_x, grid_y, grid_z):
    ...
    t_00 = tf.tile(tf.expand_dims((grid_x1f - grid_x) *
                                  (grid_y1f - grid_y), axis=-1), [1, 1, 1, 3])
    t_01 = tf.tile(tf.expand_dims((grid_x - grid_x0f) *
                                  (grid_y1f - grid_y), axis=-1), [1, 1, 1, 3])
    t_10 = tf.tile(tf.expand_dims((grid_x1f - grid_x) *
                                  (grid_y - grid_y0f), axis=-1), [1, 1, 1, 3])
    t_11 = tf.tile(tf.expand_dims((grid_x - grid_x0f) *
                                  (grid_y - grid_y0f), axis=-1), [1, 1, 1, 3])
...
def flow_gather_tf(truth, flow):
    Z, Y, X = tf.meshgrid(tf.range(tf.shape(flow)[0]), tf.range(
        tf.shape(flow)[1]), tf.range(0, tf.shape(flow)[2]), indexing='ij')
    return bilerp(truth, tf.clip_by_value(flow[..., 0] + tf.cast(X, tf.float32), 0, tf.cast(tf.shape(truth)[2], tf.float32)-1.0001),
                  tf.clip_by_value(flow[..., 1] + tf.cast(Y, tf.float32),
                                   0, tf.cast(tf.shape(truth)[1], tf.float32)-1.0001),
                  tf.cast(Z, tf.int32))


def flow_patches(truth, flow, patchsize):
    flow = tf.tile(tf.reshape(flow, [tf.shape(flow)[0], tf.shape(flow)[
                   1], tf.shape(flow)[2], 1, 1, 2]), [1, 1, 1, patchsize, patchsize, 1])
    flow = tf.transpose(flow, [0, 1, 3, 2, 4, 5])
    flow = tf.reshape(flow, [tf.shape(flow)[0], tf.shape(
        flow)[1] * patchsize, tf.shape(flow)[3] * patchsize, 2])
...
def dumb_tf_demosaic(mosaic):
    ...
    g11 = (mpad[:, 1:-3:2, 2:-2:2]+mpad[:, 3:-1:2, 2:-2:2] +
           mpad[:, 2:-2:2, 1:-3:2]+mpad[:, 2:-2:2, 3:-1:2]) / 4
    b11 = (mpad[:, 1:-3:2, 1:-3:2]+mpad[:, 3:-1:2, 1:-3:2] +
           mpad[:, 1:-3:2, 3:-1:2]+mpad[:, 3:-1:2, 3:-1:2]) / 4
    g22 = (mpad[:, 2:-2:2, 3:-1:2]+mpad[:, 4::2,  3:-1:2] +
           mpad[:, 3:-1:2, 2:-2:2]+mpad[:, 3:-1:2, 4::2]) / 4
    r22 = (mpad[:, 2:-2:2, 2:-2:2]+mpad[:, 4::2,  2:-2:2] +
           mpad[:, 2:-2:2, 4::2]+mpad[:, 4::2,  4::2]) / 4
...
def psnr_tf_batch_col_safe(estimate, truth):
    ...
    denom = tf.reduce_sum(
        tf.where(msq == 0., tf.zeros_like(msq), tf.ones_like(msq)))
...
def atan2(x, y, epsilon=1.0e-12):
    ...
    angle = tf.where(tf.logical_and(tf.less(x, 0.0),
                                    tf.greater_equal(y, 0.0)), tf.atan(y/x) + pi, angle)
    angle = tf.where(tf.logical_and(tf.less(x, 0.0),
                                    tf.less(y, 0.0)), tf.atan(y/x) - pi, angle)
    angle = tf.where(tf.logical_and(tf.equal(x, 0.0), tf.greater(
        y, 0.0)), 0.5*pi * tf.ones_like(x), angle)
    angle = tf.where(tf.logical_and(tf.equal(x, 0.0), tf.less(
        y, 0.0)), -0.5*pi * tf.ones_like(x), angle)
    angle = tf.where(tf.logical_and(tf.equal(x, 0.0),
                                    tf.equal(y, 0.0)), tf.zeros_like(x), angle)
    return angle


def vector2hsv_tf(flow):
    radius = tf.sqrt(tf.square(flow[..., 0])+tf.square(flow[..., 1]))
    radius = radius / tf.tile(tf.expand_dims(tf.expand_dims(tf.reduce_max(
        radius, axis=[1, 2]), axis=1), axis=1), (1, tf.shape(radius)[1], tf.shape(radius)[2]))
...
def pullaway_loss(batch):
    sh = tf.shape(batch)
    b = sh[0]
    b0 = tf.tile(tf.reshape(
        batch, [sh[0], 1, sh[1], sh[2], sh[3]]), [1, b, 1, 1, 1])
    b1 = tf.tile(tf.reshape(
        batch, [1, sh[0], sh[1], sh[2], sh[3]]), [b, 1, 1, 1, 1])
...
#   b1 = tf.reshape(b1, [b*b, sh[1],sh[2],sh[3]])
    ...
    x, y = tf.meshgrid(tf.range(tf.shape(dists)[0]), tf.range(
        tf.shape(dists)[1]), indexing='ij')
